...
//		System.out.println("Min cost evaluated: " + minCost);
	 ...
	 */
	public Population crossoverPopulation2(Population population, List<FogDevice> fogDevices, List<? extends Cloudlet> cloudletList) {
...
//		population.printPopulation();
	...
	public Population crossoverPopulation(Population population, List<FogDevice> fogDevices, List<? extends Cloudlet> cloudletList) {
		...
		List<Individual> newPopulation = new ArrayList<Individual>();
		// Loop over current population by fitness
		for (int populationIndex = 0; populationIndex < populationSize/2; populationIndex++) {
			Individual parent1 = population.getFittest(populationIndex);
			...
			if (this.crossoverRate >=Math.random() ) {
				...
				// Find second parent
				Individual parent2 = population.getFittest(Service.rand(0, (populationSize/2)-1));;
				...
				parent2 = population.getFittest(Service.rand(populationSize/2, populationSize-1));
				offspring = crossoverRandom(parent1, parent2);
				if(parent2.getFitness() <= calcFitness(offspring, fogDevices, cloudletList)) {
					newPopulation.add(offspring);
				} else {
		...
		for (int populationIndex =populationSize/2; populationIndex < populationSize; populationIndex++) newPopulation.add(population.getFittest(populationIndex));
		population.getPopulation().clear();
		population.setPopulation(newPopulation);
		return population;
	}
	...
	public Population searchFood(Population population, List<FogDevice> fogDevices, List<? extends Cloudlet> cloudletList) {
		ArrayList<Individual> neightborhood=new ArrayList<Individual>();
		Individual individual;
		Individual newIndividual; 
		Individual maxIndividual;
		individual = population.getFittest(0);
		maxIndividual=new Individual(individual.getChromosomeLength());
		double maxFitness;
		...
		for (int populationIndex =0; populationIndex < elitismCount; populationIndex++) {
				...
				for(int j=0;j<50;j++) {
					...
					for(int geneIndex = 0; geneIndex < individual.getChromosomeLength(); geneIndex++) newIndividual.setGene(geneIndex, individual.getGene(geneIndex));
					
					newIndividual.setGene(Service.rand(0, cloudletList.size()-1), Service.rand(0, fogDevices.size() - 1));
					
					calcFitness(newIndividual, fogDevices, cloudletList);
					
					if(newIndividual.getFitness()>maxFitness) {
						maxIndividual=newIndividual;
						maxFitness=newIndividual.getFitness();
						
					}		
				}
				
				for(int geneIndex = 0; geneIndex < individual.getChromosomeLength(); geneIndex++) individual.setGene(geneIndex, maxIndividual.getGene(geneIndex));
		}
		
		
		for (int populationIndex = (int)(0.85*populationSize); populationIndex < population.size(); populationIndex++) {
			individual = population.getFittest(populationIndex);
			
			for(int geneIndex = 0; geneIndex < individual.getChromosomeLength(); geneIndex++) individual.setGene(geneIndex,  Service.rand(0, fogDevices.size()-1));
		}
	...
	public Population changeClone(Population population,List<FogDevice> fogDevices, List<? extends Cloudlet> cloudletList) {
		...
		while(currentIndividual<population.size()&&nextIndividual<population.size()) {
			...
			if(isClone==true) {
				for(int geneIndex = 0; geneIndex < second.getChromosomeLength(); geneIndex++) second.setGene(geneIndex,  Service.rand(0, fogDevices.size()-1));
				nextIndividual++;
	...
	public boolean isCloneExist(Population population,List<FogDevice> fogDevices, List<? extends Cloudlet> cloudletList) {
		int currentIndividual=0;
		int nextIndividual=1;
		Individual first;
		Individual second;
